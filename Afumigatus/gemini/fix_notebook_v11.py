...
# IMPORTANT FIX: When writing raw strings in Python, string formatting escapes \n but 
# nbformat expects a literal block of string without unescaped newlines embedded in 
# the middle of a literal string if we are writing the python code to string literal arrays.
# nbformat will automatically convert the \n inside code into correct JSON string elements
# However, if we write it inside a python generator, we must double escape \\n so it generates \n correctly in the python source representation.
# Actually, the simplest fix is to just generate the string using literal "\n" directly in the nbformat code block as simple code string!
