...
for p in all_snpeff:
    ...
    with open(p, 'r') as f:
        for line in f:
            ...
            ref = vparts[3]
            alt = vparts[4]
            # Key definition: Chromosome, Position, Alt Allele (to distinct exact mutations)
...
print(f"Number of resistant-specific SNPs (present in ALL resistant, absent in ALL susceptible): {len(resistant_specific_snps)}")

# Output the results
out_file = "resistant_specific_snps.tsv"
...
")
    ...
    for snp in sorted(list(resistant_specific_snps), key=lambda x: (x[0], int(x[1]))):
        f.write(f"{snp[0]}	{snp[1]}	{snp[2]}	{snp[3]}
")

print("Uploading tabular result to Galaxy...")
await gxy.put(out_file, output="Resistant Specific SNPs", ext="tabular")
print("Done!")
"""

markdown_content = """# Step 4: Identifying Resistant-Specific SNPs
...
6. Upload the final list of variants as a tabular file directly to the active Galaxy history.
"""

nb.cells.append(nbf.v4.new_markdown_cell(markdown_content))
nb.cells.append(nbf.v4.new_code_cell(code))

with open('resistant_specific_snps.ipynb', 'w') as f:
    nbf.write(nb, f)

print("Created notebook: resistant_specific_snps.ipynb")
